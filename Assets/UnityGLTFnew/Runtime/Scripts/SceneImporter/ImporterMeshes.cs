...
namespace UnityGLTF
{
	public partial class GLTFSceneImporter
	{
		...
		protected virtual async Task ConstructMesh(GLTFMesh mesh, int meshIndex, CancellationToken cancellationToken)
		{
			...
			if (anyHadDraco)
			{
				float loaddracotime = Time.realtimeSinceStartup; ;
				await ConstructDracoMesh(mesh, meshIndex, cancellationToken); 
				Debug.Log("draco from start to done" + (Time.realtimeSinceStartup - loaddracotime));
				return;
			}
#else
			if (anyHadDraco)
			...
			{
				//Debug.Log("  mesh.Primitives.Count "+ mesh.Primitives.Count+ ", "+i);
				...
				//Debug.Log(" await CreateMaterials(primitive); " );
...
#pragma warning restore 0219
		...
		private async Task MeshOptDecodeBuffer(GLTFRoot root)
		{
			if (root.BufferViews == null)
				return;
			float meshoptloadtime = Time.realtimeSinceStartup;
			...
			//Debug.Log("MeshOptDecodeBuffer: "+( Time.realtimeSinceStartup - meshoptloadtime));
		}
#endif
