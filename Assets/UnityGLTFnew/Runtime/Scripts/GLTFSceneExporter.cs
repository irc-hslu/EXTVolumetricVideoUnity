...
namespace UnityGLTF
{
	...
	public partial class GLTFSceneExporter
	{
		...
		private void GLTFSceneExporterOnAfterNodeExport(GLTFSceneExporter exporter, GLTFRoot gltfRoot, Transform transform, Node node)
		{
			if (node.Extensions != null && !node.Extensions.ContainsKey(VolumetricVideo_Factory.EXTENSION_NAME))
				node.AddExtension(VolumetricVideo_Factory.EXTENSION_NAME, VV);
		}
		VolumetricVideo_Factory vvf;
		EXT_volumetricvideo VV;
		public void RegisterVV(string codecV, string urii, string mime, string medianame)
		{
			//	_exportOptions.AfterPrimitiveExport += GLTFSceneExporterOnAfterPrimitiveExport;
			//	AfterSceneExport += GLTFSceneExporterOnAfterSceneExport;
			AfterNodeExport += GLTFSceneExporterOnAfterNodeExport;
			DeclareExtensionUsage(VolumetricVideo_Factory.EXTENSION_NAME, false);
			Media.Alternative.Track tt = new Media.Alternative.Track();
			tt.codecs = codecV;
			tt.track = "track";
			Media.Alternative alt = new Media.Alternative(urii, mime, tt);

			Media mm = new Media();
			mm.alternatives.Add(alt);
			MPEG_media mediaext = new MPEG_media(medianame, mm);


			vvf = new VolumetricVideo_Factory();
			VV = new EXT_volumetricvideo(mediaext, _root);

			//_root.Scenes[0].AddExtension(VolumetricVideo_Factory.EXTENSION_NAME, VV);
			//GLTFProperty.TryRegisterExtension(vvf);
...
*/
		...
		public void SaveGLTFandBinVolo(string path, string fileName, List<Mesh> meshList = null, List<Texture> texturesList = null, bool isAnimation = false)
		{
...
//			beforeSceneExportMarker.End();
			...
			_root.Scene = ExportScene(fileName, _rootTransforms, meshList, texturesList, true);
			_root.Nodes[0].AddExtension(VolumetricVideo_Factory.EXTENSION_NAME, VV);
			if (ExportAnimations)
			{
				ExportAnimation();
			}
...
//			exportGltfMarker.End();
		...
		public void SaveGlbVolo(string path, string fileName, List<Mesh> meshList = null, List<Texture> texturesList = null, bool isgltfNode=false, bool isAnimation = false)
		{
			var fullPath = GetFileName(path, fileName, ".glb");
			var dirName = Path.GetDirectoryName(fullPath);
			if (dirName != null && !Directory.Exists(dirName))
...
//				beforeSceneExportMarker.End();
				...
				_root.Scene = ExportScene(fileName, _rootTransforms, meshList, texturesList, isgltfNode);
				_root.Nodes[0].AddExtension(VolumetricVideo_Factory.EXTENSION_NAME, VV);
...
//			exportGltfMarker.End();
		...
		private SceneId ExportScene(string name, Transform[] rootObjTransforms, List<Mesh> meshList = null, List<Texture> textureList = null, bool isgltfNode = false)
		{
	//		exportSceneMarker.Begin();
			...
			{ 
				if (textureList != null)
				{ 
					var tf = rootObjTransforms[0];
					int countGO = 0;
					string nameFORMAT = "D3";
					if (meshList.Count > 1000) nameFORMAT = "D4"; 
					for  (int j= 0;j< meshList.Count;j++)
					{
						GameObject initialTransformGO = GameObject.Instantiate(tf.gameObject) as GameObject; 
						GameObject.Destroy(initialTransformGO.GetComponent("PlayObj"));

						initialTransformGO.name = countGO.ToString(nameFORMAT);
						initialTransformGO.GetComponent<MeshFilter>().sharedMesh = meshList[j];
						if (tf.gameObject.GetComponent<Renderer>().material != null)
							initialTransformGO.GetComponent<MeshRenderer>().material = GameObject.Instantiate(tf.gameObject.GetComponent<Renderer>().material) as Material;
						else {
                            Material newMaterial = Resources.Load("videoplay_material", typeof(Material)) as Material;
							initialTransformGO.GetComponent<MeshRenderer>().material = newMaterial;
                        }
                        initialTransformGO.GetComponent<MeshRenderer>().material.mainTexture = textureList[j];

						scene.Nodes.Add(ExportNode(initialTransformGO.transform)); 
						countGO++;
					}
				}
				else if (isgltfNode)
				{
					...
					foreach (var ms in meshList)
					{
						...
						if (initialTransformGO.GetComponent("PlayObj") != null)
							GameObject.Destroy(initialTransformGO.GetComponent("PlayObj"));
						else if (initialTransformGO.GetComponent("VolPlayer") != null)
							GameObject.Destroy(initialTransformGO.GetComponent("VolPlayer"));
						initialTransformGO.name = countGO.ToString(nameFORMAT);
						initialTransformGO.GetComponent<MeshFilter>().sharedMesh = ms;
                        if (tf.gameObject.GetComponent<Renderer>().material != null)
                            initialTransformGO.GetComponent<MeshRenderer>().material = GameObject.Instantiate(tf.gameObject.GetComponent<Renderer>().material) as Material;
                        else
                        {
				...
				{
					scene.Nodes.Add(ExportNode(rootObjTransforms[0])); 
					foreach (var ms in meshList)
					{
						VV.meshAccessorList.Add(ExportMesh(ms));
					} 
				} 
			}
			else
			{
				Debug.Log("meshList null");
